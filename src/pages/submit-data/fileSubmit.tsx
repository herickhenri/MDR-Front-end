	...
	return (
		<div className="hover:shadow hover:shadow-black/30 rounded transition-all p-2">
			<div className="flex items-center gap-2 text-purple-500">
				...
				{MDR?.length && <span>MDRs: {MDR.length}</span>}
				<div className="ml-auto">
					<button onClick={loadFile}>
						<FileArrowUpIcon
							size={24}
							className="size-10 p-2 rounded text-emerald-500 cursor-pointer hover:bg-emerald-500 hover:text-white transition-colors"
						/>
					</button>
			...
			<div className="flex gap-4 text-yellow-900">
				...
				<button onClick={submitSectors}>
					<UploadSimpleIcon
						size={24}
						className="size-10 p-2 rounded text-yellow-500 cursor-pointer hover:bg-yellow-500 hover:text-white transition-colors"
					/>
				</button>
			</div>
		</div>
